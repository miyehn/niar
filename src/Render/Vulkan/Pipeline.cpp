...
VkGraphicsPipelineCreateInfo PipelineState::getPipelineInfoTemplate()
{
	...
	if (useColorBlending && !colorBlendAttachments.empty())
	{
...
void GraphicsPipelineBuilder::build(VkPipeline &outPipeline, VkPipelineLayout &outPipelineLayout, const std::string& debugName)
{
	// shader stages
...
void RayTracingPipelineBuilder::build(VkPipeline &outPipeline, VkPipelineLayout &outPipelineLayout, const std::string& debugName)
{
	// shader stages
...
void PipelineBase::destroy()
{
	...
	{
		vkDestroyPipelineLayout(Vulkan::Instance->device, layout, nullptr);
		layout = VK_NULL_HANDLE;
	}
}

void GraphicsPipeline::build(const std::string& debugName)
{
	if (pipeline != VK_NULL_HANDLE) LOG("Rebuilding GraphicsPipeline `%s`", debugName.c_str())

	builder.build(pipeline, layout, debugName);
...
void ComputePipeline::build(const std::string& debugName)
{
	if (pipeline != VK_NULL_HANDLE) LOG("Rebuilding ComputePipeline `%s`", debugName.c_str())
...
void RayTracingPipeline::build(const std::string& debugName)
{
	if (pipeline != VK_NULL_HANDLE) LOG("Rebuilding RayTracingPipeline `%s`", debugName.c_str())
